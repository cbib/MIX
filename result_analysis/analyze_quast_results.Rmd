...
g<-ggplot(myco.m[variable %in% c("N50","npredicted.genes_300.bp","ncontigs","Largest.contig")],aes(x=merger,y=value,colour=merger,group=merger))+geom_point(,position=position_jitter(width=0.05),alpha=0.9)+geom_boxplot(alpha=0.5,size=0.4,outlier.size=0)+facet_grid(variable~species,scale="free")+ggtitle("Mycoplasmas Assemblies")+xlab("Assembler type")+theme_bw()+ylab("")+theme(axis.text.x = element_text(angle = +45, hjust = 1.1))
