...
ggplot(na.omit(all.combined.quasts[,list(specie,merger,N50.rank,NA50)]),aes(x=merger,y=NA50,colour=merger,group=merger,size=N50.rank<=rank.thr,shape=N50.rank<=rank.thr))+geom_point(position=position_jitter(width=0.2,height=0.05),alpha=0.9)+geom_boxplot(alpha=0.5,size=0.4,outlier.size=0)+facet_wrap(~specie,scale="free")+ggtitle("NA50")+scale_size_discrete(range=c(small.size,large.size))
...
ggplot(na.omit(all.combined.quasts[,list(specie,merger,N50.rank,round(Duplication.ratio,digits=2))]),aes(x=merger,y=V4,colour=merger,size=N50.rank<=rank.thr,shape=N50.rank<=rank.thr,group=merger))+geom_point(position=position_jitter(width=0.1,height=0),alpha=0.9)+geom_boxplot(alpha=0.5,size=0.4,outlier.size=0)+geom_hline(y=1,colour="firebrick",linetype="dashed")+facet_wrap(~specie,scale="free")+ggtitle("Duplication ratio")+scale_size_discrete(range=c(small.size,large.size))
